...
async function criarHold(req, res) {
  try {
    const { date, time, usuario_id, especialidade } = req.body;
    const user_id = req.user?.id ?? usuario_id;

    if (!date || !time || !user_id) {
      return res
        .status(400)
        .json({ erro: "Campos obrigatórios: date, time, usuario_id." });
    }
    ...
    if (slotInicioGuard.isBefore(agora)) {
      return res
        .status(409)
        .json({ erro: "Não é possível reservar horários passados." });
    }
    ...
    const config = await AgendaConfig.findOne({ where: { dia_semana: dow } });
    if (!config)
      return res.status(400).json({ erro: "Sem atendimento neste dia." });
    ...
    if (diffMin % passo !== 0) {
      return res
        .status(400)
        .json({ erro: "Horário não está no passo configurado." });
    }
    ...
    if (jaAgendado)
      return res.status(409).json({ erro: "Horário já agendado." });
    ...
    if (jaHold)
      return res
        .status(409)
        .json({ erro: "Horário temporariamente reservado." });
    ...
    const hold = await ReservaTemp.create({
      ...
      especialidade: espSlug, // 👈 salva aqui
    });
...
async function confirmarPagamento(req, res) {
  try {
    const { payment_ref, especialidade } = req.body;
    if (!payment_ref) {
    ...
    if (!hold) {
      return res.status(404).json({
        erro: "Reserva temporária não encontrada ou expirada.",
      });
    }

    // 🔁 normaliza especialidade final:
    // 1) tenta pegar do hold
    // 2) se vier vazio por algum motivo, tenta do body (caso você queira mandar também)
    let espFinal = hold.especialidade || null;

    if (!espFinal && especialidade) {
      const raw = String(especialidade)
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
      ...
      if (
        ...
        espFinal = "emagrecimento";
      } else if (raw.includes("intoler")) {
        espFinal = "intolerancias";
      } else {
        espFinal = "clinica";
      }
    }
      ...
      agendamentoCriado = await Agendamentos.create(
        {
          ...
          especialidade: espFinal, // 👈 JOGA PRO AGENDAMENTO
        },
        { transaction: t }
      );
    ...
    // e-mail de confirmação (como já estava)
    try {
    ...
    if (e.name === "SequelizeUniqueConstraintError") {
      const existente = await Agendamentos.findOne({
        where: { idempotency_key: req.body?.payment_ref },
      });
      if (existente) {
        await ReservaTemp.destroy({
          where: { payment_ref: req.body?.payment_ref },
        });
        return res.status(200).json({
          msg: "Pagamento já confirmado.",
