...
const questionsFallback = [
  { pergunta: 'Como marcar sua consulta?', resposta: 'Você pode marcar sua consulta entrando em contato por telefone ou e-mail.' },
  { pergunta: 'Qual é o horário de funcionamento?', resposta: 'De segunda a sexta, das 8h às 18h.' },
  { pergunta: 'A Dra. Natália aceita convênios?', resposta: 'Não aceitamos convênios diretamente, mas fornecemos recibo para reembolso.' },
  { pergunta: 'Quais são os métodos de pagamento aceitos?', resposta: 'Aceitamos dinheiro, PIX e cartões de débito e crédito.' },
  { pergunta: 'Posso ter um reembolso do convênio?', resposta: 'Sim! Basta solicitar o recibo para apresentar ao seu plano de saúde.' },
];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // fallback silencioso
        if (mounted) {
          setItems(questionsFallback.map((q, i) => ({ id: i+1, ...q })));
          setError("Falha ao carregar FAQ online. Exibindo conteúdo padrão.");
        }
      }
    })();
  ...
  return (
    <div className="faq-contato-wrapper">
      ...
      <main className="faq-main">
        ...
        {/* Mensagens simples sem alterar classes do layout */}
        {isLoading && <p style={{ textAlign: "center", margin: "8px 0" }}>Carregando…</p>}
        {!isLoading && error && <p style={{ textAlign: "center", margin: "8px 0", color: "#a55" }}>{error}</p>}
        {!isLoading && items?.length === 0 && (
          <p style={{ textAlign: "center", margin: "8px 0", color: "#666" }}>
